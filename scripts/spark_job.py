...
# Initialize Spark session with optimization configs
def init_spark():
    return (SparkSession.builder
            .appName("Flight Price Data Transformation")
            .config("spark.sql.shuffle.partitions", "8")  # Reduce shuffle partitions for moderate data size
            .config("spark.sql.adaptive.enabled", "true")  # Enable adaptive query execution for optimization
            .getOrCreate())

# Get min and max timestamp bounds for partitioning
...
# Read data from MySQL with dynamic partition bounds
...
def enforce_schema(df: DataFrame) -> DataFrame:
    ...
    for c, t in schema_map.items():
        if isinstance(t, tuple) and t[0] == "timestamp":
            ...
            # Here, t is a DataType, so safe to pass to cast()
...
# Clean data
def clean_data(df):
    # Drop rows with missing critical fare or location info
    df = df.dropna(subset=["Airline", "Source", "Destination", "Base Fare (BDT)", "Tax & Surcharge (BDT)"])
    
    # Fill null Total Fare with sum of Base Fare and Tax & Surcharge
    df = df.withColumn(
        "Total Fare (BDT)",
...
def write_to_postgres(df_dict, df_main, env_vars):
    ...
    for name, df in df_dict.items():
        df.write.jdbc(env_vars["POSTGRES_URL"], f"kpi_{name}", mode="overwrite", properties=pg_properties)
...
def main():
    ...
    # Cache cleaned data to optimize multiple queries
    df_cleaned.cache()
